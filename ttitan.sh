...
do
    ...
    container_id=$(docker run -d --restart always -v $mount_point:/root/.titanedge/storage --name "ttitan$i" nezha123/titan-edge)

    echo "node ttitan$i telah memulai ID containe $container_id"
